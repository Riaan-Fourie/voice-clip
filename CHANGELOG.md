...
### Added
- **Hands-free lock mode (Jarvis #273).** Press **Right-Shift + Right-Command** to toggle a hands-free recording: tap once to start recording without holding anything, tap again to stop and transcribe — so you can lean back and dictate. Hold Right-Command alone still works exactly as before (hold-to-record). If you're already mid hold-recording, the chord *promotes* it to locked, so releasing the keys no longer stops it. Implemented as a small state machine (idle / hold / locked) with the three CGEventTap handlers serialized under a lock so a chord can't race a plain press.
- **Lightning grows to consume the whole screen the longer you speak (Jarvis #273).** The recording overlay is now a full-screen, transparent, click-through window; the cheek bolts' reach, count and spread ramp with the recording's elapsed time (`FILL_SECONDS`), from a spark halo around the body to a 360° storm that fills the screen after ~16s of continuous talking — still crackling with your voice level on top. Resets each recording.
- **Animated mascot visualizer with voice-reactive lightning (Jarvis #273).** The recording overlay is now an animated mascot (`assets/mascot.gif`) whose cheeks throw lightning that scales with your live mic level — a lazy spark or two when quiet, a full crackling storm when you're loud, with an arc leaping between the cheeks on peaks. Bolts are drawn live in Cocoa (`NSBezierPath`, midpoint-displacement), rooted to per-frame cheek anchor points baked offline into `assets/mascot_cheeks.json` (so the app needs no image analysis at runtime). Falls back to a pulsing dot if the assets are missing, so recording is never left with no visible indicator. Replaces the old green/yellow/red VU bars.

### Added
- **Audio observability for diagnosing mid-recording silence (Jarvis #277).** Set `VOICECLIP_DEBUG_AUDIO=1` and every recording logs a compact per-second RMS trace of the captured WAV to the debug log — so an intermittent "audio went silent after ~3s" bug (seen on AirPods Pro 3) is self-diagnosing from the log alone: a run of near-zero seconds after real speech is the signature. `VOICECLIP_DEBUG_AUDIO=wav` additionally dumps each recording to `~/.voice-clip/debug_recordings/`. Off by default, zero overhead when unset.

### Fixed
- **A quick Right-Cmd tap no longer starts a recording that never stops (Jarvis #327).** The debounce rejected the *release* edge, but the press edge had already opened the mic — so any tap shorter than `MIN_HOLD_DURATION` (0.15s) stranded the recorder in the recording state. It kept capturing room audio until some later, unrelated release cleared the threshold, and the whole buffer was transcribed as one stray sentence. The symptom read as *"VoiceClip truncated my dictation"* rather than *"VoiceClip is stuck"*, which is why it survived so long: measured over the live debug log, **27 runaways out of 1346 hold recordings, median 25s, worst 217s**. A sub-threshold release now dispatches a new `on_cancel` handler that stops the recorder and discards the audio (a tap that short is not speech). Locked recordings are untouched — they still stop only on the next toggle.
- **The overlay now rebuilds itself when its window wedges (Jarvis #323).** The mascot had been invisible for **six days** while recording, the hotkey and transcription all worked and every log line looked perfect. The overlay `NSPanel` is created once and never rebuilt; after enough uptime and Space churn macOS left it permanently unable to join any Space, and it kept accepting `setAlphaValue_`/`orderFrontRegardless` while the WindowServer displayed nothing. `show()` was doing everything right — right screen (#275), right level (#284), alpha 1.0 — onto a window that could no longer be shown. `show()` now asks the WindowServer (via `CGWindowListCopyWindowInfo`, not `NSWindow.isVisible()`, which a wedged window happily lies about) whether the panel actually landed on the active Space ~0.35s after showing, and rebuilds it if not — at most once per recording, and never when the window is healthy, so there is no flicker in the normal case. The wedge and the rebuild's outcome are both logged, so this can never again be invisible in the log. Mascot frames are now cached process-wide so a rebuild costs no disk I/O.
- **AirPods mic went silent after ~4s of a recording — App Nap (Jarvis #277).** On longer dictations the Bluetooth (HFP) mic dropped to digital silence a few seconds in, so everything after was lost (the buffer stayed full-length, just zeros). Root cause: under the app's Cocoa run loop macOS App-Naps the process and mutes the Bluetooth input — the built-in mic and a no-run-loop process were both immune, which is why it only bit on AirPods in the live app. The recorder now holds a latency-critical `NSProcessInfo` activity assertion for the lifetime of each stream, which keeps the process out of App Nap and the mic live. Witnessed: AirPods went from dying at t=4s to strong audio through t=12s under the run loop.
- **Recording lightning no longer fills the screen and blocks reading (Jarvis #276).** The #273 "grows to fill the whole screen" ramp scaled bolt reach by the screen *diagonal*, so during longer dictations the storm covered the text being dictated over. Bolts are now bounded to a small halo around the mascot (reach scales with the mascot size, not the screen), with a modest fan and bolt count — still crackling with your voice, but never covering the screen. Witnessed at max reach + loudest level: halo stays around Pikachu, page text readable.
- **Mascot overlay was invisible on multi-monitor setups (Jarvis #275).** The #273 full-screen overlay is created once, on whichever display was `NSScreen.mainScreen()` at the first recording, and was never repositioned — so on a dual-monitor setup the whole overlay could be stranded on the *external* display while you dictate on the built-in, and the mascot never appeared (audio still recorded fine). The pre-#273 small window accidentally dodged this by ignoring `screen.frame().origin`. Now `show()` re-pins the window + content view to the current `mainScreen().frame()` every time, so the overlay always lands on the display you're actually using. Witnessed by screencapturing both displays before/after the fix.
- **Wedge watchdog now escalates on repeated gate-resets (jarvis-system #6, Jarvis #273).** The #187 re-exec only fired when the transcribe *thread* was still alive (a true native Metal wedge). But the app was twice observed sitting dead for HOURS while the gate merely "leaked … resetting" on a loop — the wedge was elsewhere (a hung `recorder.stop`, or a stale thread handle), so the classifier kept picking the cheap reset and never escalated. Now `RESET_ESCALATE_THRESHOLD` (3) consecutive gate-resets with no successful transcription in between forces a full `os.execv` re-exec — the same recovery a native wedge already gets. A genuinely successful transcription clears the counter.
